...
class LagopusBridge(object):
    ...
    def __init__(self, ryu_app, name, dpid, b_type=BRIDGE_TYPE_VLAN):
...
class LagopusManager(object):
    ...
    def __init__(self, ryu_app, bridge_mappings):
        ...
        for item in raw_data:
            b_name = item["name"]
            b_type = (BRIDGE_TYPE_PHYS if b_name in phys_bridge_names
                      else BRIDGE_TYPE_VLAN)
            bridge = LagopusBridge(ryu_app, b_name, item["dpid"], b_type)
    ...
    def bridge_add_port(self, bridge, port):
        if port.bridge is None:
            ...
            self.lagopus_client.bridge_add_port(bridge.name, port.name, ofport)
            bridge.add_port(port, ofport)
