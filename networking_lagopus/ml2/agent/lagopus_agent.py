...
class LagopusCache(dict):
    ...
    def __init__(self, resource_cls, cache_change_hook):
        self.resource_cls = resource_cls
        self.cache_change_hook = cache_change_hook

    def add(self, name, *args):
        # this method is called when initialization to register already
        # exist resources. cache_change_hook will be called once at
...
class LagopusManager(object):
    ...
    def __init__(self, ryu_app, bridge_mappings):
        ...
        # channel
        self.channels = LagopusCache(lg_lib.LagopusChannel, self._rebuild_dsl)
        ...
        # port
        self.ports = LagopusCache(lg_lib.LagopusPort, self._rebuild_dsl)
        ...
        # bridge
        self.bridges = LagopusCache(lg_lib.LagopusBridge, self._rebuild_dsl)
    ...
    def _rebuild_dsl(self):
        # TODO(oda): just for test now. not replace actual dsl yet.
        path = "/tmp/lagopus-test.dsl"
    ...
    def bridge_add_port(self, bridge, port):
        if port.bridge is None:
            ...
            # TODO(oda): a bit dirty hack. rethink.
            self._rebuild_dsl()
